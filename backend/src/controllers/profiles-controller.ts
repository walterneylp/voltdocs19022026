...
export const getProfiles = async (req: Request, res: Response) => {
  ...
  if (supabaseAdmin && req.accessToken) {
    ...
    const tenantId =
      (payload?.user_metadata as { tenant_id?: string } | undefined)?.tenant_id ??
...
export const patchUserGroup = async (req: Request, res: Response) => {
  const client = req.supabase;
  if (!client) return res.status(500).json({ error: "Missing supabase client" });
  const { id } = req.params;
  if (!id) return res.status(400).json({ error: "Grupo invalido." });
...
export const postUserGroupMember = async (req: Request, res: Response) => {
  if (!supabaseAdmin) {
...
export const deleteUserGroupMember = async (req: Request, res: Response) => {
  if (!supabaseAdmin) {
    res.status(500).json({ error: "Missing SUPABASE_SERVICE_ROLE_KEY" });
    return;
  }
...
export const getUserGroupMembers = async (req: Request, res: Response) => {
  const client = req.supabase;
  if (!client) return res.status(500).json({ error: "Missing supabase client" });
