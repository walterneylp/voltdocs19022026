...
import {
  ...
  listCompanyProfiles
} from "../repositories/company-profiles-repo.js";
...
export const postCompanyProfile = async (req: Request, res: Response) => {
  const client = req.supabase;
  if (!client) return res.status(500).json({ error: "Missing supabase client" });
...
export const patchCompanyProfile = async (req: Request, res: Response) => {
  const client = req.supabase;
  if (!client) return res.status(500).json({ error: "Missing supabase client" });
...
export const removeCompanyProfile = async (req: Request, res: Response) => {
  const client = req.supabase;
  if (!client) return res.status(500).json({ error: "Missing supabase client" });
  const profileId = z.string().uuid().parse(req.params.id);
