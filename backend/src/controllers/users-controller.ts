...
const getProfileOrFail = async (req: Request, res: Response, id: string) => {
  if (!supabaseAdmin) {
    res.status(500).json({ error: "Missing SUPABASE_SERVICE_ROLE_KEY" });
    return null;
  }
...
const hasUserActivity = async (req: Request, userId: string) => {
  const client = req.supabase;
  ...
  if ((ticketCount ?? 0) > 0) return true;
...
export const deleteUser = async (req: Request, res: Response) => {
  ...
  const tenantId =
    (userData.user.user_metadata as { tenant_id?: string } | undefined)?.tenant_id ?? null;
  if (tenantId) {
    await supabaseAdmin
      .from("user_group_members")
      .delete()
