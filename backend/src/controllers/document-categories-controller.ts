...
} from "../repositories/document-categories-repo.js";
...
export const patchDocumentCategory = async (req: Request, res: Response) => {
  const client = req.supabase;
  if (!client) return res.status(500).json({ error: "Missing supabase client" });
  const { id } = req.params;
  if (!id) return res.status(400).json({ error: "Categoria invalida." });
...
export const deleteDocumentCategoryById = async (req: Request, res: Response) => {
  const client = req.supabase;
  if (!client) return res.status(500).json({ error: "Missing supabase client" });
  const { id } = req.params;
  if (!id) return res.status(400).json({ error: "Categoria invalida." });
