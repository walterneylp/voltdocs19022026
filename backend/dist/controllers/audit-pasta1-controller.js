...
import { createAuditRun, finishAuditRun, getAuditConfigMeta, insertAuditItemEvidenceExclusion, insertAuditItemEvidence, insertAuditResults, listAuditConfigItems, listAuditEvidencesForItems, listAuditItemEvidences, listAuditItemEvidenceIds, listAuditItemEvidenceExclusions, listLatestAuditResults, upsertAuditConfigItems, upsertAuditConfigMeta } from "../repositories/audit-pasta1-repo.js";
import { clearDocumentChunks, insertDocumentChunks, searchDocumentChunks } from "../repositories/document-chunks-repo.js";
...
const scoreToStatus = (score) => {
    if (score >= 80)
        return "PROVAVELMENTE_ATENDIDO";
    if (score >= 40)
        return "PROVAVELMENTE_PARCIAL";
    if (score === 0)
        return "PROVAVELMENTE_PENDENTE";
    return "INCONCLUSIVO";
};
...
const autoLinkDocuments = async (req, items) => {
    const client = req.supabase;
    if (!client)
        throw new Error("Missing supabase client");
    ...
    (versions ?? []).forEach((version) => {
        if (version.file_name)
            fileNameByDoc.set(version.document_id, version.file_name);
    });
    await Promise.all(items.map(async (item) => {
        const existingDocIds = new Set(await listAuditItemEvidenceIds(client, item.item_id));
        const excludedDocIds = new Set(await listAuditItemEvidenceExclusions(client, tenantId, item.item_id));
        const keywordsToMatch = item.palavras_chave && item.palavras_chave.length > 0
            ? item.palavras_chave
            : [item.nome];
        ...
        try {
            ...
            chunks
                ...
                .forEach((chunk) => {
                if (!excludedDocIds.has(chunk.document_id))
                    ragDocIds.add(chunk.document_id);
            });
        }
        catch {
            // ignore rag fallback
...
export const getAuditPasta1Config = async (req, res) => {
    const client = req.supabase;
    if (!client)
...
export const getAuditPasta1Results = async (req, res) => {
    const client = req.supabase;
    if (!client)
        return res.status(500).json({ error: "Missing supabase client" });
    const tenantId = await getTenantId(req);
    if (!tenantId)
        return res.status(400).json({ error: "Tenant nao encontrado." });
    const data = await listLatestAuditResults(client, tenantId);
    res.json({ data });
};
export const postAuditPasta1Evidence = async (req, res) => {
    const client = req.supabase;
    if (!client)
        return res.status(500).json({ error: "Missing supabase client" });
    const tenantId = await getTenantId(req);
...
export const deleteAuditPasta1Evidence = async (req, res) => {
    const client = req.supabase;
    if (!client)
        return res.status(500).json({ error: "Missing supabase client" });
    const tenantId = await getTenantId(req);
...
export const getAuditPasta1Evidences = async (req, res) => {
    const client = req.supabase;
    if (!client)
        return res.status(500).json({ error: "Missing supabase client" });
...
export const runAuditPasta1 = async (req, res) => {
    const client = req.supabase;
    if (!client)
        return res.status(500).json({ error: "Missing supabase client" });
    const tenantId = await getTenantId(req);
    ...
    const results = await Promise.all(items.map(async (item) => {
        if (item.item_id === "1.1") {
            const requiredMap = {
                ...
                endereco: Boolean(companyProfile?.address_street ||
        ...
        const keywordMatches = (allDocuments ?? []).filter((doc) => includesKeyword(`${doc.title ?? ""} ${doc.category ?? ""}`, item.palavras_chave ?? []));
        let ragMatches = [];
        try {
            const query = `${item.nome}. ${item.categoria}. ${item.palavras_chave?.join(", ") ?? ""}`;
            const [embedding] = await createEmbeddings([query]);
            ...
            ...itemEvidences.map((evidence) => {
                ...
                if (doc?.title && includesKeyword(doc.title, keywords)) {
                    keywords.forEach((keyword) => {
                        if (includesKeyword(doc.title ?? "", [keyword]))
                            matchedKeywords.add(keyword);
                    });
                }
                if (doc?.category && includesKeyword(doc.category, keywords)) {
                    keywords.forEach((keyword) => {
                        if (includesKeyword(doc.category ?? "", [keyword]))
                            matchedKeywords.add(keyword);
                    });
                }
        ...
        return {
            ...
            recomendacoes: matchedKeywords.size === 0
...
export const postAuditPasta1Index = async (req, res) => {
    const client = req.supabase;
    if (!client)
        return res.status(500).json({ error: "Missing supabase client" });
    const tenantId = await getTenantId(req);
    ...
    (versions ?? []).forEach((version) => {
        if (version.file_name)
            versionByDoc.set(version.document_id, version.file_name);
    });
