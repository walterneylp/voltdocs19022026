...
import { getMe, listAssets, listProfiles, listTickets, listUserGroups, updateTicket } from "../lib/api";
import "../styles/chamados.css";
import "../styles/chamados-novo.css";

export function ChamadosAtualizar() {
  ...
  useEffect(() => {
    let isMounted = true;
    Promise.all([getMe(), listTickets(), listAssets(), listProfiles(), listUserGroups()])
      .then(([me, tickets, assetList, profileList, groupList]) => {
        if (!isMounted) return;
        ...
        const equipment = assetList.find((asset) => asset.id === ticket.equipment_id);
        if (equipment) {
          setSelectedEquipment(equipment);
          setEquipmentQuery(equipment.tag);
        }
        const profileMap = new Map(
          profileList.map((profile) => [profile.id, profile.name ?? profile.email ?? "Sem nome"])
        );
        const groupMap = new Map(groupList.map((group) => [group.id, group.name]));
        const assignedGroupIds = ticket.assigned_group_ids ?? [];
        const extraTechs: Array<{ id: string; name: string }> = [];
        const assignedGroups: string[] = [];
        assignedGroupIds.forEach((id) => {
          const profileName = profileMap.get(id);
          if (profileName) {
            extraTechs.push({ id, name: profileName });
          } else if (groupMap.has(id)) {
            assignedGroups.push(id);
          }
        });
        const primaryTech = ticket.assigned_to_id
          ? profileMap.get(ticket.assigned_to_id)
          : null;
        const techList: Array<{ id: string; name: string }> = [];
        if (ticket.assigned_to_id && primaryTech) {
          techList.push({ id: ticket.assigned_to_id, name: primaryTech });
        }
        extraTechs.forEach((tech) => {
          if (!techList.find((item) => item.id === tech.id)) {
            techList.push(tech);
          }
        });
        setSelectedTechnicians(techList);
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const [primaryTech, ...extraTechs] = selectedTechnicians;
      const mixedAssignments = [
        ...selectedGroups,
        ...extraTechs.map((tech) => tech.id)
      ];
      const finalDescription = observation.trim()
      ...
      await updateTicket(ticketId, {
        ...
        assigned_to_id: primaryTech?.id ?? null,
  ...
  return (
    <div className="chamados-novo">
      ...
      <section className="chamados-modal" role="dialog" aria-label="Atualizar Chamado">
        ...
        <div className="chamados-modal-body">
          ...
          <div className="chamados-field chamados-status">
            <label htmlFor="chamado-status">Status Atual</label>
            <div className="chamados-select">
              <select
                id="chamado-status"
                value={status}
              ...
              >
                ...
                <option value="ABERTO">REABERTO</option>
              </select>
              <LucideIcon name="chevron-down" className="chamados-select-icon" />
            </div>
          </div>
          ...
          <div className="chamados-field">
            <label htmlFor="chamado-observacao">Observação do Técnico</label>
            <input
              id="chamado-observacao"
              type="text"
