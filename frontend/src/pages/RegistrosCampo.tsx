...
export function RegistrosCampo() {
  ...
  const pendingUpdates = filteredUpdates.filter((update) => {
    const status = (update.status ?? "").toLowerCase();
    if (status === "processado") return false;
    if (update.event_report && !showEvent) return false;
    if (!update.event_report && !showTicket) return false;
    return true;
  });
  ...
  const groupedPending = useMemo(() => {
    ...
    const map = new Map<
      string,
      { key: string; latest: typeof pendingUpdates[number]; updates: typeof pendingUpdates }
    >();
        ...
        <section className="rc-main">
          <div className="rc-header">
            ...
            <div className="rc-tabs">
              ...
              <span className="rc-count">{pendingUpdates.length} registros</span>
            </div>
          </div>
          ...
          <div className="rc-filter">
            ...
            <div className="rc-filter-options">
              <label>
                <input
                  type="checkbox"
                  checked={showTicket}
                  onChange={(event) => setShowTicket(event.target.checked)}
                />
                Com chamado aberto
              </label>
              <label>
                <input
                  type="checkbox"
                  checked={showEvent}
                  onChange={(event) => setShowEvent(event.target.checked)}
                />
                Sem chamado (registro em campo)
              </label>
            </div>
          </div>
          ...
          {groupedPending.length === 0 && !error ? (
            <div className="rc-empty">Nenhum registro pendente encontrado.</div>
          ) : null}
            ...
            <article className="rc-card" key={group.key}>
              <div className="rc-card-header">
                ...
                <div className="rc-meta">
                  ...
                  <p className="rc-meta-code">
                    Referência: {group.latest.code ?? group.latest.base_path ?? group.key}
                  </p>
